...
import { Props, applyDomainIfNotAbsolute } from "./component.tsx"

const template = (props: { contentType: string, base64: string }) => `data:${props.contentType};base64,${props.base64}`

export async function dataUrl (urlOrPathname: string) {
  const ext = path.extname(urlOrPathname)
  const contentType = getContentType(ext)
  if (!contentType) throw new Error(`Could not determine content type for ${urlOrPathname}`)
...
export async function transformImages (data: Props, handleImage: (img: string) => string | Promise<string>): Promise<Props> {
  const jobs = data.jobs
  if (data.downloadIcon) data.downloadIcon = await handleImage(data.downloadIcon)
  if (!jobs) return data
...
export function transform (data: Props): Promise<Props> {
  return transformImages(data, (img) => dataUrl(applyDomainIfNotAbsolute(data.domain, img)))
}